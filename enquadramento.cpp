...
void Enquadramento:: enviar(char * buffer, int bytes, int tipMsg){
	...
	//if(bytes >= _min_bytes && bytes <= _max_bytes ){
		...
		if(1==tipMsg){
			for(int i= 0; i<k; i++){
				printf(" tYYYYY %x    ", quadro[k]);;
			}
		}
		//cout<< "Mensagem flag: "<<quadro[k-1]<<endl;
...
int Enquadramento:: receber(char * buffer){
	...
	while(not c){
		...
		i++;
		//cout<<"Mensagem recebida: "<<  i <<carac<<endl;

  	}
	//cout<<"Mensagem recebidaaaaaa: "<<buffer<<n_bytes<<endl;

		//for (int k = 0; k < n_bytes; k++) {
//printf("\ncrc buffer: %X e a posi%d: \n", buffer[k],k);
//}
