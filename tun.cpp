...
tun::tun(char *dev, char * ip, char * dst) {
	tty_fd_tun = tun_alloc(dev);

	if (tty_fd_tun > 0) {
		printf("Interface %s_%d criada.\n", dev, tty_fd_tun);
		if (set_ip(dev, ip, dst ) < 0) {
  			perror("Ocorreu um erro ao configurar a interface tun");
		}
	} else {
		printf("Erro\n");
	}
...
int tun::read(char * buffer, int size) {
	int n = ::read(tty_fd_tun, buffer, size);
	return n;
}

int tun::write(char * buffer, int size) {
	int n = ::write(tty_fd_tun, buffer, size);
	printf("%X\n", buffer);
	return n;
}
